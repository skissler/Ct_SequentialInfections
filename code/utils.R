...
get_long_output <- function(fitlist, nmvec, idname="id"){

	for(nm in nmvec){
		if(!(nm %in% names(fitlist))){
			print("ERROR: at least one column in nmvec does not exist")
			return()
		}
	}
	...
	out <- filter(out, iteration>=0)
	out <- rename_with(out,function(x){return(idname)},"id")
	return(out)
}

get_wide_output <- function(fitlist, nmvec){
	...
	nindiv <- dim(fitlist$tp)[2]
	out <- tibble(iteration=-Inf, id=-Inf) 
	for(nm in nmvec){

		temp <- as_tibble(fitlist[[nm]], .name_repair="unique") 
		if(ncol(temp)==1){
				temp <- Reduce(cbind,
					lapply(1:nindiv, function(x){return(temp)}))
				names(temp) <- as.character(1:nindiv)
				temp <- as_tibble(temp) 
			} else if(ncol(temp)==nindiv){
				names(temp) <- as.character(1:nindiv)
			} else {
				print("ERROR: at least one column requires pre-processing")
				return()
			}
		temp <- temp %>% 
			mutate(iteration=1:n()) %>% 
			pivot_longer(-iteration) 
		names(temp) <- c("iteration","id",nm)
		temp$id <- as.numeric(temp$id)

		out <- full_join(out, temp, by=c("iteration","id"))
	}
	out <- filter(out, iteration>=0)
	return(out)
}

make_tidy <- function(wide_output){
	topivot <- setdiff(names(wide_output), c("iteration","id"))
	out <- wide_output %>% 
		pivot_longer(topivot)
	return(out)
}

clean_names <- function(tidy_output){
	out <- tidy_output %>% 
		mutate(name=case_when(
			grepl("delta",name)~"Delta",
			grepl("omicron",name)~"Omicron",
			grepl("exposed",name)~"Exposed"
			))
	return(out)
}

clean_names_full <- function(tidy_output){
	out <- tidy_output %>% 
		mutate(GreekLineage=case_when(
			grepl("delta",name)~"Delta",
			grepl("omicron",name)~"Omicron",
			grepl("other",name)~"Other"
			)) %>% 
		mutate(IsExposed=case_when(
			grepl("exposed",name)~1, TRUE~0)) %>% 
		mutate(IsRecent=case_when(
			grepl("recent",name)~1, TRUE~0)) %>% 
		mutate(IsHighTiter=case_when(
			grepl("hightiter",name)~1, TRUE~0))
	return(out)
}

clean_variant_names <- function(tidy_output){
	out <- tidy_output %>% 
		mutate(name=case_when(
			grepl("delta",name)~"Delta",
			grepl("omicron",name)~"Omicron",
...
make_greek_lineage <- function(df){
	out <- df %>% 
		mutate(GreekLineage=case_when(
			(grepl("B.1.617.2",Lineage) | grepl("AY",Lineage))~"Delta",
			(grepl("B.1.1.529",Lineage) | grepl("BA",Lineage))~"Omicron",
			grepl("None",Lineage) ~ "None",
			TRUE~"Other"
			))
	return(out)
}

plot_chains <- function(fitlist_chain, varname, transform="identity"){

	if(!(transform%in%c("identity","exp","log"))){
		print("ERROR: Unrecognized transform")
		return(NA)
	}

	var_df <- tibble(
		chain1=fitlist_chain[,1,varname],
		chain2=fitlist_chain[,2,varname],
		chain3=fitlist_chain[,3,varname],
		chain4=fitlist_chain[,4,varname]) %>% 
		mutate(Iteration=1:n())

	out <- var_df %>% 
		pivot_longer(-Iteration, names_to="name", values_to="Value") %>% 
		mutate(Transform=transform) %>% 
		mutate(Value=case_when(
			Transform=="identity"~Value,
			Transform=="exp"~exp(Value),
			Transform=="log"~log(Value)
			)) %>% 
		ggplot(aes(x=Iteration, y=Value, col=name)) + 
			...
			labs(title=paste0(varname," (",transform,")"))
...
get_latest_pos <- function(df){
	latestposdf <- df %>% 
		filter(CtT1<global_pars[["lod"]]) %>% 
		group_by(PersonID, InfectionEvent) %>% 
		arrange(desc(TestDate)) %>% 
		slice(1) %>% 
		select(PersonID, InfectionEvent, LastPosDate=TestDate) %>% 
		# keep those with >=2 infection events,
		group_by(PersonID) %>% 
		mutate(NInf=n_distinct(InfectionEvent)) %>% 
		filter(NInf>1) %>% 
		arrange(PersonID, InfectionEvent, LastPosDate) %>% 
		mutate(LatestPosDate=lag(LastPosDate)) %>% 
		filter(!is.na(LatestPosDate)) %>% 
		select(PersonID, InfectionEvent, LatestPosDate) %>% 
		ungroup() 

	df <- df %>% 
		left_join(latestposdf, by=c("PersonID","InfectionEvent")) %>% 
		ungroup() 
...
make_test_date_index <- function(ct_dat){
	minctdf <- ct_dat %>% 
		arrange(IQVIAID, TestDate) %>%
		group_by(IQVIAID) %>%
		mutate(index=1:n()) %>%
		filter(CtT1==min(CtT1)) %>%
		slice(1) %>%
		select(IQVIAID, TestDate) %>%
		rename(MinCtTestDate=TestDate)

	out <- ct_dat %>% 
		left_join(minctdf, by="IQVIAID") %>%
...
make_test_date_index_infevent <- function(ct_dat){
	minctdf <- ct_dat %>% 
		arrange(PersonID, InfectionEvent, DaysSinceDetection) %>%
		group_by(PersonID,InfectionEvent) %>%
		mutate(index=1:n()) %>%
		filter(CtT1==min(CtT1)) %>%
		slice(1) %>%
		select(PersonID,InfectionEvent,DaysSinceDetection) %>%
		rename(MinCtTestDate=DaysSinceDetection)

	out <- ct_dat %>% 
		left_join(minctdf, by=c("PersonID","InfectionEvent")) %>%
		mutate(TestDateIndex=DaysSinceDetection-MinCtTestDate) %>%
		select(-MinCtTestDate)
...
plot_ct_fit_split_basic <- function(params_df, global_pars, indiv_data, ctalpha=0.01, basecol="black", xlim=c(NA,NA),nsplits=4,shade_ids=c(),ntraces=100){
	with(as.list(global_pars),{
	...
	params_df %>% 
		...
		imap(~ ggplot(.x) + 
			...
			geom_segment(aes(x=tp+wr, y=lod, xend=Inf, yend=lod), alpha=ctalpha) + 
			# Plot data:
			geom_point(data=indiv_data_split[[.y]], aes(x=t, y=y), col=basecol, size=0.5) + 
			theme_minimal() + 
			theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), legend.title=element_blank(), axis.text.x=element_text(angle=90, hjust=1, vjust=0.5, size=8), axis.text.y=element_text(size=8)) + 
			labs(x="Time since min Ct (days)", y="Ct") + 
			scale_y_reverse(breaks=c(40,30,20,10), labels=c("(-)","30","20","10"), sec.axis=sec_axis(~convert_Ct_logGEML(.), name=expression(log[10]~RNA~copies/ml))) + 
...
# Ct = (-3.60971 * (geml - log10(250))) + 40.93733
...
get_titercat <- function(df, titercuts, skipfirstinf=1){
	titercuts <- sort(titercuts)
	df$titercat <- NA_real_
	if(skipfirstinf==1){
		for(indexA in rev(1:length(titercuts))){
			df <- df %>% 
				mutate(titercat=case_when(
					CumulativeExposureNumber<=1~1,
					(CumulativeExposureNumber>1 & Titer<=titercuts[indexA])~(indexA+1),
					TRUE~titercat
					))
			}
		df <- df %>% 
			replace_na(list(titercat=(length(titercuts)+2))) %>% 
			mutate(titercat=as.numeric(titercat)) %>% 
			mutate(titercat=case_when((is.na(Titer) & CumulativeExposureNumber>1)~0,TRUE~titercat))
		return(df) 	
	} else {
		for(indexA in rev(1:length(titercuts))){
			df <- df %>% 
				mutate(titercat=case_when(
					(Titer<=titercuts[indexA])~as.numeric(indexA),
					TRUE~titercat
					))
		}
		df <- df %>% 
			replace_na(list(titercat=(length(titercuts)+1))) %>% 
			mutate(titercat=as.numeric(titercat)) %>% 
			mutate(titercat=case_when((is.na(Titer))~0,TRUE~titercat))
		return(df) 
	}
}
